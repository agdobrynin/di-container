...
- [Добавить определения](#добавить-определения-через-итерируемую-коллекцию) через метод:
...
Загрузка из отдельных файлов конфигураций сервисов контейнера.

Предусмотрены режимы загрузки:
- отслеживать уникальные идентификаторы контейнера.
...
## Добавить определения через итерируемую коллекцию.
Добавляет итерируемую коллекцию определений в контейнер. Предусмотрено два варианта:
- отслеживать уникальные идентификаторы контейнера.
- перезапись ранее добавленных идентификаторов контейнера.
...
```
Аргументы:
- `$definitions` итерируемая коллекция определений с идентификаторами контейнера;
...
Идентификатор контейнера должен быть уникален в рамках текущего контейнера, если идентификатор не уникален будет выброшено исключение `\Kaspi\DiContainer\Interfaces\Exceptions\ContainerAlreadyRegisteredExceptionInterface`.

Определение может быть представлено ввиде [хэлпер функции](01-php-definition.md#объявления-для-определений-контейнера) или
объекта.

> [!WARNING]
> Рекомендуется использовать файлы конфигурации определений через класс-строитель `DiContainerBuilder`,
> так как определения установленные напрямую в контейнер не будут [скопилированы для продуктивной среды](#компиляция-контейнера) (_prod env_).
> 
...
;
```
Установка в контейнер зависимостей где-то в коде приложения:
```php
use App\Services\Others;

$qux = new Qux();

$container->set('service.qux', $qux);
```
Получение где-то в коде приложения из контейнера зависимостей:
...
final class Foo {
    ...
    {
        $service = $container->get('service.qux');    
    }
}
```
