...
## Autowire
Атрибут позволяет конфигурировать PHP класс как сервис контейнера
и может применяться к PHP классу или к параметру метода (функции).

Атрибут позволяет отдельно конфигурировать теги и сетер-методы PHP класса
...
> [!NOTE]
> Пустая строка в параметре `$id` может быть интерпретирована контейнером как полное имя класса – **fully qualified class name**:
...
При указании атрибута к параметру метода (функции) значение в `$id` может быть указано как полное имя класса которое может быть получено контейнером или представлено как пустая строка.
В случае если `$id` будет пустой строкой, то контейнер попытается сформировать `$id` на основе типа параметра (_type hint_).
...
> [!IMPORTANT]
> При указании атрибута к параметру метода или функции **не учитываются значения указанные для параметров** `Autowire`:
> - `\Kaspi\DiContainer\Attributes\Autowire::$isSingleton` – singleton сервис
> - `\Kaspi\DiContainer\Attributes\Autowire::$tags` – теги класса
> - `$resetter` – сброс состояния объекта
>
